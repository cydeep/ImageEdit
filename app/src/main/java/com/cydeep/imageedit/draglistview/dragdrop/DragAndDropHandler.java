...
import com.cydeep.imageedit.draglistview.coreutil.OnItemMovedListener;
...
import com.cydeep.imageedit.draglistview.view.DynamicListViewWrapper;
...
public class DragAndDropHandler implements TouchEventHandler {
    ...
    private int notSwapItemCount = 0;
    ...
    private void switchIfNecessary() {
        ...
        long aboveItemId = position - 1 - mWrapper.getHeaderViewsCount() >= 0
                           ...
                           : INVALID_ID;
        int tempBelowPosition = position + 1 - mWrapper.getHeaderViewsCount();
        long belowItemId = (tempBelowPosition < mAdapter.getCount() - notSwapItemCount && tempBelowPosition > -1)
                           ? mAdapter.getItemId(tempBelowPosition)
                           : INVALID_ID;
        ...
        if (switchView != null) {
//            int height = switchView.getHeight() > mHoverDrawable.getIntrinsicHeight()?switchView.getHeight():mHoverDrawable.getIntrinsicHeight();
            if (Math.abs(deltaY) > switchView.getHeight()) {
    ...
    public void setNotSwapItemCount(int notSwapItemCount) {
        this.notSwapItemCount = notSwapItemCount;
    }
    ...
    private class LSwitchViewAnimator implements SwitchViewAnimator {
        ...
        private class AnimateSwitchViewOnPreDrawListener implements ViewTreeObserver.OnPreDrawListener {
            ...
            public boolean onPreDraw() {
                ...
                View tempView = getViewForId(mMobileItemId);
                if (tempView != null) {
                    mMobileView = tempView;
                    mMobileView.setVisibility(View.INVISIBLE);
                } else {
                    System.out.println("--------");
                }
    ...
    private class ScrollHandler implements AbsListView.OnScrollListener {
        ...
        void handleMobileCellScroll() {
            ...
            } else if (hoverViewTop + hoverHeight >= (height - hoverHeight / 2) && offset + extent < range) {
                mWrapper.smoothScrollBy(scrollPx, 0);
            }
        }
        ...
        private void checkAndHandleLastVisibleCellChange() {
            ...
            int tempBelowPosition = position + 1 - mWrapper.getHeaderViewsCount();
            long switchItemId = (tempBelowPosition < mAdapter.getCount() - notSwapItemCount && tempBelowPosition > -1)
                                ? mAdapter.getItemId(tempBelowPosition)
                                : INVALID_ID;
            View switchView = getViewForId(switchItemId);
            if (switchView != null) {
    ...
    private class SettleHoverDrawableAnimatorListener extends AnimatorListenerAdapter implements ValueAnimator.AnimatorUpdateListener {
        ...
        public void onAnimationEnd(final Animator animation) {
            ...
            if (mHoverDrawable != null && mHoverDrawable.getBitmap() != null) {
                mHoverDrawable.getBitmap().recycle();
                mHoverDrawable.setCallback(null);
            }
            mHoverDrawable = null;
            mMobileView = null;
            mMobileItemId = INVALID_ID;
